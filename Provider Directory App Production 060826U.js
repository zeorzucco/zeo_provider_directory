  ...
  function ensureFloridaCityCentroids() {
    if (!floridaCityCentroids) {
      ...
      var e, t, r, a, o, i, n, l = zipLookup ? .z || {},
        ...
        s = new Map;
      for (e of Object.values(l)) !Array.isArray(e) || e.length < 3 || (a = "number" == typeof(a = e[0]) && zipLookup ?
        .c ? .[a] || "") && (t = "number" == typeof e[1] ? e[1] / d : null, r = "number" == typeof e[2] ? e[2] / d :
        null, Number.isFinite(t)) && Number.isFinite(r) && (a = normalizeCity(a), (o = s.get(a) || {
        sumLat: 0,
        sumLng: 0,
        count: 0
  ...
  function getInitialsFromPracticeName(e) {
    var e = String(e || "").trim();
    return e ? (((e = e.split(/\s+/).filter(Boolean))[0] ? .[0] || "") + ((1 < e.length ? e[1] ? .[0] : e[0] ? .[1]) ||
      "")).toUpperCase() : "??"
  }
  ...
  async function loadData() {
    ...
    floridaCitySet = null, floridaCityCentroids = null, catalog.procedures = Array.from(document.querySelectorAll(
      ".cms-procedures .w-dyn-item")).map(e => ({
    ...
    }).filter(e => e.ProviderID && e.ProviderID.trim() || e.DoctorName && e.DoctorName.trim() || e.PracticeName && e.PracticeName
      .trim());
    let r = (CONFIG.allowedStates || []).map(e => String(e).toUpperCase());
    r.length && (catalog.providers = catalog.providers.filter(e => r.includes(String(e.State || "").toUpperCase()))),
      typeaheadItems = buildTypeaheadItems(catalog), console.log("CMS loaded:", {
  ...
  function showSuggestions(e, o = -1) {
    els.termSuggestions.innerHTML = "", e.length ? (e.forEach((t, e) => {
      var r = document.createElement("div"),
        e = (r.className = "suggestion", r.setAttribute("role", "option"), r.setAttribute("data-id", t.id), r.setAttribute(
          "data-type", t.type), r.setAttribute("aria-selected", e === o ? "true" : "false"), document.createElement(
          "div")),
        a = (e.className = "suggestion__name", e.textContent = t.name, document.createElement("div"));
      a.className = "suggestion__type", a.textContent = t.label, r.appendChild(e), r.appendChild(a), r.addEventListener(
        "mousedown", e => {
  ...
  function commitSelection(e) {
    selectedTerm = {
        ...
        id: e.id,
        name: e.name
      }, lastCommittedTermValue = e.name, els.termInput.value = e.name, els.termError.classList.add("hidden"),
      updateClearButtons()
  }
  ...
  function updateClearButtons() {
    var e;
    els.termClearBtn && (e = 0 < String(els.termInput.value || "").trim().length, els.termClearBtn.classList.toggle(
      "hidden", !e)), els.locClearBtn && (e = 0 < String(els.locInput.value || "").trim().length, els.locClearBtn.classList
      .toggle("hidden", !e))
  }

  function showTermErrorIfNeeded() {
    var e, t = String(els.termInput.value || "").trim();
    !t || selectedTerm && selectedTerm.id ? els.termError.classList.add("hidden") : (0 === (e = buildSuggestions(t)).length ?
      (z_track("directory_term_invalid_no_match", {
        typed: normTerm(t).slice(0, 80),
        typed_len: t.length
      }), els.termError.textContent = "No matching procedures or specialties. Try a different term.") : (z_track(
        "directory_term_invalid_not_selected", {
          typed: normTerm(t).slice(0, 80),
          typed_len: t.length,
          suggestions_count: e.length
        }), els.termError.textContent = "Please select a procedure or specialty from the list."), els.termError.classList
      .remove("hidden"))
  }

  function toTitleCase(e) {
    return String(e || "").trim().toLowerCase().split(/\s+/).filter(Boolean).map(e => e.charAt(0).toUpperCase() + e.slice(
      1)).join(" ")
  }

  function resolveFloridaLocation(e) {
    var t, r, a, o, e = normalizeStr(e);
    return isZip(e) ? (t = zipLookup ? .z ? .[e], Array.isArray(t) && !(t.length < 3) && (r = "number" == typeof zipLookup ?
      .s ? zipLookup.s : 1e4, a = "number" == typeof(a = t[0]) && zipLookup ? .c ? .[a] || "") ? (o = "number" ==
      typeof t[1] ? t[1] / r : null, t = "number" == typeof t[2] ? t[2] / r : null, {
        mode: "zip",
        input: e,
  ...
  function filterProviders() {
    ...
    var e = catalog.providers.filter(e => !0 === e.Active && t.has(String(e.State || "").toUpperCase())).filter(e => !!
      selectedTerm && ("procedure" === selectedTerm.type ? Array.isArray(e.procedure_ids) && e.procedure_ids.includes(
        selectedTerm.id) : "specialty" === selectedTerm.type && Array.isArray(e.specialty_ids) && e.specialty_ids.includes(
        selectedTerm.id)));
    ...
    return 0 < (o = Number.isFinite(a.lat) && Number.isFinite(a.lng) ? e.map(e => {
      var t = Number(e.lat),
        r = Number(e.lng);
      return Number.isFinite(t) && Number.isFinite(r) ? (t = milesBetween(a.lat, a.lng, t, r), { ...e,
        _distMiles: t
      }) : null
    }).filter(Boolean).filter(e => e._distMiles <= r) : o).length ? (o.sort((e, t) => {
      var r = e.PreferredProvider ? 1 : 0,
        a = t.PreferredProvider ? 1 : 0;
      return r != a || (a = Number(e._distMiles), r = Number(t._distMiles), Number.isFinite(a) && Number.isFinite(
        r) && a !== r) ? a - r : (a = normalizeStr(e.DoctorName), r = normalizeStr(t.DoctorName), 0 !== (a = a.localeCompare(
        r)) ? a : normalizeStr(e.PracticeName).localeCompare(normalizeStr(t.PracticeName)))
    }), o) : ((e = e).sort((e, t) => {
      var r = e.PreferredProvider ? 1 : 0,
        a = t.PreferredProvider ? 1 : 0;
      return r != a ? a - r : (a = normalizeStr(e.DoctorName), r = normalizeStr(t.DoctorName), 0 !== (a = a.localeCompare(
        r)) ? a : normalizeStr(e.PracticeName).localeCompare(normalizeStr(t.PracticeName)))
    }), e)
  }

  function getProviderSpecialtyLabel(e) {
    return selectedTerm && "specialty" === selectedTerm.type ? specialtyNameById[selectedTerm.id] || selectedTerm.name ||
      "" : (e = Array.isArray(e.specialty_ids) ? e.specialty_ids[0] : "") && specialtyNameById[e] || ""
  }

  function renderResults(e = !0) {
    e && (els.listPane.innerHTML = "", renderedCount = 0, activeProviderId = null);
    var e = filteredProviders.length,
      t = (els.resultsCount.textContent = 1 === e ? "1 result" : e + " results", els.emptyState.classList.toggle(
        "hidden", 0 !== e), filteredProviders.slice(renderedCount, renderedCount + CONFIG.resultsPageSize)),
  ...
  function createProviderCard(t) {
    ...
    var o, i, n = getCardImageUrl(t),
      l = (n ? ((l = document.createElement("img")).className = "card__photoImg", l.src = n, l.alt =
        `${t.DoctorName||t.PracticeName||"Provider"} photo`, l.loading = "lazy", l.onerror = () => {
          a.innerHTML = "";
          var e = document.createElement("div");
          e.className = "card__photoFallback", e.textContent = getInitialsFromPracticeName(t.PracticeName), a.appendChild(
            e)
        }, a.appendChild(l)) : ((n = document.createElement("div")).className = "card__photoFallback", n.textContent =
        getInitialsFromPracticeName(t.PracticeName), a.appendChild(n)), document.createElement("div")),
      n = (l.className = "card__names", document.createElement("div")),
      d = (n.className = "card__doctor card__doctor--primary", n.textContent = t.DoctorName || "",
        getProviderSpecialtyLabel(t)),
      s = document.createElement("div"),
      c = (s.className = "card__specialty", s.textContent = d, document.createElement("div")),
      n = (c.className = "card__practice card__practice--secondary", c.textContent = t.PracticeName || "", t.DoctorName &&
        l.appendChild(n), d && l.appendChild(s), t.PracticeName && l.appendChild(c), document.createElement("div")),
      s = (n.className = "card__badgeCol", t.PreferredProvider && ((d = document.createElement("div")).className =
        "badge", d.textContent = "Preferred Provider", n.appendChild(d)), r.appendChild(a), r.appendChild(l), r.appendChild(
        n), document.createElement("div")),
      c = (s.className = "card__row card__row--details", document.createElement("div")),
      l = (c.className = "card__details", t.Phone && ((d = document.createElement("div")).className = "detail", (l =
        document.createElement("span")).className = "detail__icon", l.innerHTML = iconPhoneSvg(), (n = document.createElement(
        "a")).className = "detail__value", n.href = "tel:" + normalizePhoneForTel(t.Phone), n.addEventListener(
        "click", e => e.stopPropagation()), n.textContent = t.Phone, d.appendChild(l), d.appendChild(n), c.appendChild(
        d)), t.AddressLine1 || t.City || t.State || t.ZIP),
      l = (l && ((n = document.createElement("div")).className = "detail detail--address", (d = document.createElement(
          "span")).className = "detail__icon", d.innerHTML = iconPinSvg(), (l = document.createElement("div")).className =
        "detail__stack", t.AddressLine1 && ((i = document.createElement("div")).className = "addr__line", i.textContent =
          t.AddressLine1, l.appendChild(i)), (i = document.createElement("div")).className =
        "addr__line addr__line--muted", i.textContent = [t.City, t.State, t.ZIP].filter(Boolean).join(", "), l.appendChild(
          i), (i = document.createElement("div")).className = "addr__wrap", (o = document.createElement("div")).className =
        "addr__viewMap", o.innerHTML = iconGlobeSvg() + '<span class="addr__viewMapText">Map</span>', i.appendChild(l),
        i.appendChild(o), n.appendChild(d), n.appendChild(i), n.setAttribute("role", "button"), n.tabIndex = 0, n.addEventListener(
          "click", e => {
            window.matchMedia("(max-width: 900px)").matches && (e.stopPropagation(), focusProviderOnMap(t))
          }), n.addEventListener("keydown", e => {
          !window.matchMedia("(max-width: 900px)").matches || "Enter" !== e.key && " " !== e.key || (e.preventDefault(),
            e.stopPropagation(), focusProviderOnMap(t))
        }), c.appendChild(n), (l = document.createElement("a")).className = "directions", l.href = getDirectionsUrl(t),
        l.target = "_blank", l.rel = "noreferrer", l.addEventListener("click", e => e.stopPropagation()), l.textContent =
        "Get directions", c.appendChild(l)), t.Website && ((o = document.createElement("div")).className = "detail",
        (d = document.createElement("span")).className = "detail__icon", d.innerHTML = iconGlobeSvg(), (i = document.createElement(
          "a")).className = "detail__value", i.href = t.Website, i.target = "_blank", i.rel = "noreferrer", i.addEventListener(
          "click", e => e.stopPropagation()), i.textContent = "Website", o.appendChild(d), o.appendChild(i), c.appendChild(
          o)), t.HighlightText && ((n = document.createElement("div")).className = "card__highlight", n.textContent =
        String(t.HighlightText).slice(0, 500), c.appendChild(n)), s.appendChild(c), document.createElement("div")),
      d = (l.className = "card__row card__row--actions", document.createElement("button"));
    return d.className = "button", d.type = "button", d.textContent = "Request appointment", d.addEventListener("click",
      e => {
  ...
  function setActiveCard(t) {
    activeProviderId = t, els.listPane.querySelectorAll(".card").forEach(e => e.classList.toggle("card--active", e.getAttribute(
      "data-provider-id") === t))
  }
  ...
  function getDirectionsUrl(e) {
    ...
    return t || ((t = [String(e ? .AddressLine1 || "").trim(), String(e ? .City || "").trim(), String(e ? .State || "")
        .trim(), String(e ? .ZIP || "").trim()
      ].filter(Boolean)).length ? "https://www.google.com/maps/search/?api=1&query=" + encodeURIComponent(t.join(", ")) :
  ...
  function buildMapPopupContent(t) {
    ...
    if (i.textContent = getInitialsFromPracticeName(n || "Provider"), o) {
      ...
      e.className = "map-card__avatar-img", e.src = o, e.alt = t.DoctorName || "Provider", e.loading = "lazy", e.addEventListener(
        "error", () => {
    ...
    var n = document.createElement("div"),
      ...
      l = (o.className = "map-card__title", o.textContent = t.DoctorName || t.PracticeName || "",
        getProviderSpecialtyLabel(t)),
      d = document.createElement("div"),
      s = (d.className = "map-card__specialty", d.textContent = l, document.createElement("div")),
      o = (s.className = "map-card__practice", s.textContent = t.DoctorName && t.PracticeName || "", n.appendChild(o),
        l && n.appendChild(d), s.textContent && n.appendChild(s), r.appendChild(a), r.appendChild(n), document.createElement(
          "div")),
      ...
      a = (s.className = "map-card__metaLine", s.textContent = t.Phone || "", d.textContent && o.appendChild(d), s.textContent &&
        o.appendChild(s), document.createElement("div")),
      n = (a.className = "map-card__actions", document.createElement("button")),
      l = (n.className = "map-card__btn map-card__btn--primary", n.textContent = "Request appointment", n.addEventListener(
        "click", e => {
          e.stopPropagation(), openJotformModal(t)
        }), document.createElement("div")),
      d = (l.className = "map-card__actionsRow", document.createElement("a"));
    return d.className = "map-card__btn map-card__btn--secondary", d.textContent = "Directions", d.href =
      getDirectionsUrl(t), d.target = "_blank", d.rel = "noopener", d.addEventListener("click", () => {
        z_track("provider_directions_click", z_ctx(t))
      }), l.appendChild(d), t.Phone && ((s = document.createElement("a")).className =
        "map-card__btn map-card__btn--secondary", s.textContent = "Call", s.href = "tel:" + normalizePhoneForTel(t.Phone),
        l.appendChild(s), s.addEventListener("click", () => {
          z_track("provider_call_click", z_ctx(t))
        })), a.appendChild(n), a.appendChild(l), e.appendChild(r), o.childElementCount && e.appendChild(o), e.appendChild(
        a), e
  }
  ...
  function loadGoogleMaps() {
    return new Promise((e, t) => {
      var r;
      window.google && window.google.maps ? e() : (r = document.getElementById("gmaps-script")) ? (r.addEventListener(
        "load", () => e()), r.addEventListener("error", () => t(new Error("Google Maps failed to load")))) : ((r =
          document.createElement("script")).id = "gmaps-script", r.async = !0, r.defer = !0, r.src =
        "https://maps.googleapis.com/maps/api/js?key=" + encodeURIComponent(CONFIG.googleMapsApiKey), r.onload =
        () => e(), r.onerror = () => t(new Error("Google Maps failed to load")), document.head.appendChild(r))
    })
  }
  ...
  function fitMapToProviders(r) {
    if (map && window.google ? .maps) {
      ...
      if (0 !== r.length) {
        ...
        0 !== e && (1 === e && t ? (map.setCenter(t), map.setZoom(CONFIG.defaultZoomOnSelect)) : (map.fitBounds(o, 40),
  ...
  function getProviderKey(e) {
    var t = e ? .ProviderID ? ? e ? .ProviderId ? ? e ? .providerId ? ? e ? .id ? ? e ? ._id ? ? e ? .slug ? ? e ? .Slug ?
      ? e ? .PracticeSlug ? ? e ? .practiceSlug ? ? null;
    return t ? String(t) : (t = [normalizeStr(e ? .DoctorName || ""), normalizeStr(e ? .PracticeName || ""),
      normalizeStr(e ? .City || ""), normalizeStr(e ? .State || "")
    ].filter(Boolean)).length ? t.join("|") : null
  }
  ...
  function focusProviderOnMap(e) {
    ...
    map && e && (r = getProviderKey(e)) && (t = markersByProviderId.get(r)) && (setActiveCard(r), map.panTo({
      lat: e.lat,
      lng: e.lng
    }), map.setZoom(CONFIG.defaultZoomOnSelect), window.matchMedia("(max-width: 768px)").matches && setMobileView(
      "map"), r = buildMapPopupContent(e), infoWindow.setContent(r), infoWindow.open({
      anchor: t,
      map: map
    }))
  }
  ...
  els.jotformFrame.src=CONFIG.jotformBaseUrl+"?"+params.toString();
  ...
  function closeModal() {
    els.modalOverlay.classList.add("hidden"), els.modalOverlay.setAttribute("aria-hidden", "true"), els.jotformFrame.src =
      "about:blank", document.body.style.overflow = ""
  }
  ...
  function formatTermForMeta(e) {
    var t;
    return e && (t = String(e.name || "").trim()) ? "procedure" === e.type ? "providers offering " + t : "specialty" ===
      e.type ? t + " providers" : "providers" : "providers"
  }
...
async function showProviderFromUrl() {
  ...
  const provider = catalog.providers.find(p =>
    String(p.Slug || "").trim().toLowerCase() === slug.trim().toLowerCase()
  );
  ...
  if (!provider) {
    ...
    els.searchMeta.textContent = "Provider not found.";
    return true;
  }
  ...
  els.resultsCount.textContent = "1 result";
  renderResults(true);

  els.searchMeta.textContent = `Showing provider: ${provider.DoctorName || provider.PracticeName}`;
  ...
  async function runSearch() {
    els.searchMeta.textContent = "", els.searchMeta.classList.add("hidden"), clearSelectionIfUserEdited(),
      showTermErrorIfNeeded(), validateSearchButton();
    let e = ++searchRunId;
    if (activeProviderId = null, pendingFitProviders = null, renderedCount = 0, els.listPane.innerHTML = "",
      clearMarkers(), selectedTerm) {
      let t = resolveFloridaLocation(normalizeStr(els.locInput.value));
      filteredProviders = filterProviders(), z_track("directory_search_submit", { ...z_ctx(),
        loc_mode: t ? .mode || "",
        loc_recognized: t ? 1 : 0
      }), z_track("directory_search_results", { ...z_ctx(),
        loc_mode: t ? .mode || "",
        loc_recognized: t ? 1 : 0,
        results_count: filteredProviders.length
      }), 0 === filteredProviders.length && z_track("directory_search_zero_results", { ...z_ctx(),
        loc_mode: t ? .mode || "",
        loc_recognized: t ? 1 : 0
      }), showResultsSection(), filteredProviders.length ? (setMapVisibility(!0), await initMapIfNeeded(), t && "zip" ===
        t.mode && t.lat && t.lng && (map.setCenter({
          lat: t.lat,
          lng: t.lng
        }), map.setZoom(10)), (a = filteredProviders.filter(e => Number.isFinite(e.lat) && Number.isFinite(e.lng) &&
          !(0 === e.lat && 0 === e.lng))).length ? renderMapPins(a) : (setMapVisibility(!1), clearMarkers()),
        setTimeout(() => {
          searchRunId === e && google.maps.event.trigger(map, "resize")
        }, 100)) : (setMapVisibility(!1), clearMarkers()), renderResults(!0), els.searchMeta.classList.remove(
        "hidden");
      var r, a = Array.isArray(CONFIG.allowedStates) && 1 === CONFIG.allowedStates.length && "FL" === String(CONFIG.allowedStates[
        0]).toUpperCase();
      t ? (r = toTitleCase(els.locInput.value), r = "zip" === t.mode || isFloridaCityName(r) ? "" + t.displayCity : r,
        a && "city" === t.mode && !isFloridaCityName(a = toTitleCase(els.locInput.value)) && (els.searchMeta.textContent =
          `Right now this directory is Florida-only. "${a}" doesn’t look like a Florida location. Try a Florida city or ZIP.`
        ), 0 === filteredProviders.length ? els.searchMeta.textContent =
        `No matches within ${CONFIG.radiusMiles||20} miles of ${r}.` : filteredProviders.some(e => normalizeCity(e.City) ===
          t.city) ? els.searchMeta.textContent =
        `Searching for: ${selectedTerm.name} within ${CONFIG.radiusMiles||20} miles of ` + r : els.searchMeta.textContent
        .startsWith("Right now this directory is Florida-only.") || (a = formatTermForMeta(selectedTerm), els.searchMeta
          .textContent = `No providers within ${CONFIG.radiusMiles||20} miles of ${r} yet. Showing ${a} in Florida.`)
      ) : els.searchMeta.textContent = "ZIP not recognized in Florida yet. Try a Florida city."
    }
  }

  function setMobileView(e) {
    var t = window.matchMedia("(max-width: 768px)").matches;
    if (t && "map" === e && (savedWindowScrollY = window.scrollY || 0, document.body.style.position = "fixed", document
        .body.style.top = `-${savedWindowScrollY}px`, document.body.style.left = "0", document.body.style.right = "0",
        document.body.style.width = "100%"), t && "list" === e) {
      t = document.body.style.top;
      document.body.style.position = "", document.body.style.top = "", document.body.style.left = "", document.body.style
        .right = "", document.body.style.width = "";
      let e = t ? Math.abs(parseInt(t, 10)) : savedWindowScrollY;
      requestAnimationFrame(() => {
        window.scrollTo(0, e || 0)
      })
    }
    isMobileMapView = "map" === e, document.body.classList.toggle("mobile-view--list", "list" === e), document.body.classList
      .toggle("mobile-view--map", "map" === e);
    t = document.getElementById("mobileViewToggle");
    t && (t.textContent = "map" === e ? "List" : "Map"), "map" === e && map && window.google ? .maps && setTimeout(() => {
      google.maps.event.trigger(map, "resize"), !activeProviderId && pendingFitProviders && pendingFitProviders.length &&
        (fitMapToProviders(pendingFitProviders), pendingFitProviders = null)
    }, 150)
  }
  ...
  e.addEventListener("click", function(ev){
    ...
    setMobileView(
      document.body.classList.contains("mobile-view--map") ? "list" : "map"
    );
  });
}
  ...
  function nudgeSearchCardForIOS() {
    var e, t;
    window.matchMedia("(max-width: 768px)").matches && (t = (t = window.visualViewport) ? t.height : window.innerHeight,
        e = document.querySelector(".search")) && (40 < (e = e.getBoundingClientRect().top - 12) && window.scrollBy({
        top: e,
        left: 0,
        behavior: "smooth"
      }), e = document.getElementById("termSuggestions")) && !e.classList.contains("hidden") && (e = e.getBoundingClientRect())
      .bottom > (t = t - 24) && window.scrollBy({
        top: e.bottom - t,
        left: 0,
        behavior: "smooth"
      })
  ...
  function wireEvents() {
    ...
    els.termInput.addEventListener("input", () => {
      ...
      els.termSuggestions.classList.contains("hidden") || ("ArrowDown" === e.key ? (e.preventDefault(), t = Math.min(
          t + 1, r.length - 1), showSuggestions(r, t)) : "ArrowUp" === e.key ? (e.preventDefault(), t = Math.max(
          t - 1, 0), showSuggestions(r, t)) : "Enter" === e.key ? (e.preventDefault(), (0 <= t && r[t] ? (
          commitSelection(r[t]), hideSuggestions(), validateSearchButton) : showTermErrorIfNeeded)()) : "Escape" ===
        e.key && hideSuggestions())
      ...
      e.preventDefault(), e.stopPropagation(), els.termInput.value = "", selectedTerm = null,
        lastCommittedTermValue = "", els.termError.classList.add("hidden"), hideSuggestions(), validateSearchButton(),
        updateClearButtons(), els.termInput.focus()
    }), els.locClearBtn && els.locClearBtn.addEventListener("click", e => {
      e.preventDefault(), e.stopPropagation(), els.locInput.value = "", validateSearchButton(), updateClearButtons(),
        els.locInput.focus()
    }), els.searchBtn.addEventListener("click", async () => {
      els.termInput.blur(), els.locInput.blur(), hideSuggestions(), window.matchMedia("(max-width: 768px)").matches &&
        setMobileView("list"), await runSearch()
